...
class TrendAnalyzer:
    ...
    def _window(self, current: Telemetry, target_tick: int, label: str) -> dict[str, Any]:
        ...
        if baseline_row is None or baseline_row["telemetry"].get("schemaVersion") not in (3, 4, 5):
            baseline_row = next(
                (
                    row for row in self.history.observations_since(target_tick)
                    if row["telemetry"].get("schemaVersion") in (3, 4, 5)
        ...
        rows = [
            ...
            if row["telemetry"].get("schemaVersion") in (3, 4, 5) and row["screeps_tick"] <= current.tick
        ]
